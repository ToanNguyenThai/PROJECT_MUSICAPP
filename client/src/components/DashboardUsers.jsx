        ...
        <motion.div key={index} className="relative w-full rounded-md flex items-center justify-between py-4 bg-lightOverlay cursor-pointer hover:bg-card hover:shadow-md">
            {
                data._id !== user?.user._id && (
                    <motion.div whileTap={{ scale: 0.75 }} className="absolute left-4 w-8 h-8 rounded-md flex items-center justify-center bg-gray-200" onClick={() => deleteUser(data._id)}>
                        <MdDelete className='text-x1 text-pink-300 hover:text-pink-600' />
                    </motion.div>
                )
            }
            ...
            <div className='w-275 min-w-[160px] text-center flex items-center justify-center gap-6 relative'>
                ...
                {isUserRoleUpdated && (
                    <motion.div
                        ...
                        <p className='text-textColor text-[16px] font-semibold'>Are you sure to mark the user as <span>{data.role === "admin" ? "member" : "admin"}?</span></p>
                        <div className='flex items-center gap-20'>
                            <motion.button whileTap={{ scale: 0.75 }} className="outline-none border-none text-sm px-4 py1 rounded-md bg-blue-200 text-black hover:shadow-md" onClick={() => updateUserRole(data._id, data.role === "admin" ? "member" : "admin")}>
                                Yes
                            </motion.button>
